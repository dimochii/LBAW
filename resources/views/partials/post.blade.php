...
<div data-post="{{$post->post_id}}"
  ...
  <header class="flex items-center relative">
    <a class="flex items-center h-8" href="{{ route('communities.show', ['id' => $post->post->community->id ?? 'unknown']) }}">
      ...
      <span class="text-xl font-light underline-effect-light">h/{{ $post->post->community->name ?? 'Unknown Community' }}</span>
    </a>
