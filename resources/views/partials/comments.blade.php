<div class="comment relative mb-3 min-w-72 max-w-full" id="c-{{ $comment->id }}">
  <div class="flex flex-row mt-5">
      <div class="min-w-[32px] mr-3 flex flex-col items-center w-[32px]">
          ...
          <a href="#c-{{ $comment->id }}" class="absolute top-[32px] bg-[#A6A6A6] hover:bg-[#4793AF] w-px hover:w-0.5 h-[calc(100%-32px)] cursor-pointer"></a>
      </div>
      <details open class="group/details mt-2 grow">
          ...
          {{-- Comment Body --}}
          <article data-text="markdown" class="font-vollkorn max-w-full prose prose-a:text-[#4793AF]/[.80] hover:prose-a:text-[#4793AF]/[1]">
              {{ $comment->content }}
          </article>
          ...
          <footer class="flex gap-x-1 items-center mt-4">
              ...
              <div>
                  ...
                  <label for="upvote-{{ $comment->id }}" class="peer-checked/upvote:fill-blue-400 cursor-pointer hover:fill-blue-400 transition-all ease-out">
              ...
              <div>
                  ...
                  <label for="downvote-{{ $comment->id }}" class="cursor-pointer peer-checked/downvote:fill-red-400 hover:fill-red-400 transition-all ease-out">
              ...
              <button data-toggle="reply-form" data-target="{{ $comment->id }}-editor">
                  <svg class="ml-2 h-[18px] w-[18px] hover:fill-blue-400 transition-all ease-out" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
          ...
          {{-- Nested Replies --}}
          <div class="replies pl-12">
              @foreach ($comment->children as $childComment)
                  @include('partials.comments', ['comment' => $childComment])
              @endforeach
          </div>
      </details>
  </div>
